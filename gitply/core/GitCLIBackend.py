...
class GitCLIBackend (object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.since = None
        if 'since' in kwargs:
            self.since = kwargs['since']
    
    def __iter__(self):
        return chain(*[self.iterate_commits(self.get_repo_log(repo, self.since)) for repo in self.repos])
    ...
    def iterate_commits(log_lines):
        ...
        for line in it:
            if line[0:7] == "Author:":
                ...
                match = GitCLIBackend.STATS_REGEX.match(line)
